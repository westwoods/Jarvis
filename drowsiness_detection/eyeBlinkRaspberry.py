...
EYE_PERCENT_TOP = 35        # 눈이 얼굴(y축 기준)에서 얼마나 위쪽에 위치하는지 결정
EYE_PERCENT_SIDE = 18       # 눈이 얼굴 중앙(x축 기준)으로부터 얼마나 옆에 위치하는지 결정
EYE_PERCENT_HEIGHT = 11     # 눈의 높이
...
EYE_THRESHOLD2 = 45  # 파이 카메라
EYE_THRESHOLD1 = 170 # 마이크로소프트 카메라
BINARY_WHITE = 255
BINARY_BLACK = 0

# 깜빡임을 인식하는 Threshold
BLINK_THRESHOLD2 = 15000    # 파이 카메라
BLINK_THRESHOLD1 = 15000    # 마이크로소프트 카메라
...
def find_eyes(gray, face):  # 위에서 흑백프레임과 얼굴을 넘겨받음.
    ...
    (_, thresh_left) = cv2.threshold(left_eye_roi, EYE_THRESHOLD2, BINARY_WHITE, cv2.THRESH_BINARY_INV)
    # 팽창
    ...
    #(_, thresh_right) = cv2.threshold(right_eye_roi, EYE_THRESHOLD2, BINARY_WHITE, cv2.THRESH_BINARY_INV)
    ...
    # 검정 색을 카운팅 한다.
    dst = cv2.inRange(thresh_left, BINARY_BLACK, 50)
    countOfBlack = cv2.countNonZero(dst)
    print('black count : ' + str(countOfBlack))

    if(countOfBlack > BLINK_THRESHOLD2):
        print('eye close')
    else:
        print('eye open')
